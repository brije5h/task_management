                ...
                <div key={i} className='flex flex-col justify-between bg-gray-800 rounded-lg p-4 hover:scale-105 cursor-pointer transition-all'>
                    ...
                    <div className='mt-2 flex items-center justify-between'>
                        <div className='border-red-500'>
                            <button className={` ${items.complete === false ? "bg-red-500" : "bg-green-800"} rounded p-2  font-semibold `}
                            onClick={()=>handleCompleteTask(items._id)} >
                                {items.complete === true ? "Completed": "Incomplete"}
                            </button>
                        </div>
                        <div className='flex gap-4'>
                            <button className={`${items.important===false? "text-red-500":"text-green-800"}`} onClick={()=> handleImpTask(items._id)}><CiHeart /></button>
